...
import { HiOutlineCog6Tooth, HiOutlineHome, HiOutlineMusicalNote, HiOutlineQueueList } from "react-icons/hi2";
        ...
        <NavLink
          ...
          className={({ isActive }) =>
            `flex min-w-16 flex-col items-center gap-1 rounded-2xl px-3 py-2 font-medium transition ${
              isActive ? "bg-white/10 text-primary shadow-soft" : "text-muted hover:bg-white/5 hover:text-primary"
            }`
          }
        >
          {({ isActive }) => (
            <>
              <span className={`flex h-8 w-8 items-center justify-center rounded-xl transition ${isActive ? "bg-glow/25 text-white shadow-glow" : "bg-white/10"}`}>
